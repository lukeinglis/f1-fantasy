...
export default async function RaceDetailPage(props: {
  ...
  if (!race) {
    return (
      <div className="garage-card">
        ...
        <Link
          href="/races"
          className="text-[var(--color-racing-red)] hover:underline mt-2 inline-block"
        >
          Back to calendar
        </Link>
      </div>
  ...
  return (
    <div className="space-y-6">
      <Link
        href="/races"
        className="inline-flex items-center gap-1 text-sm text-[var(--color-garage-metal)] hover:text-[var(--color-oil-stain)] transition-colors"
      >
      ...
      {/* Pre-season banner */}
      {preSeason && (
        <div className="garage-card px-4 py-3 text-sm text-[var(--color-garage-metal)] flex items-center gap-3">
          <span className="text-[var(--color-garage-metal)] text-lg">&#9432;</span>
          <span>
      ...
      <header className="garage-card">
        ...
        <div className="flex flex-wrap items-center gap-3 mt-3">
          <div className="text-sm text-[var(--color-oil-stain)]">{fmtDate(race.date)}</div>
          {race.resultsLocked && (
            <span className="garage-badge bg-emerald-600 text-white">
              Results final
            </span>
          )}
          {!race.resultsLocked && deadlinePassed && (
            <span className="garage-badge bg-amber-500 text-white">
              Picks closed
            </span>
          )}
          {!deadlinePassed && !preSeason && (
            <span className="garage-badge bg-[var(--color-garage-metal)] text-white">
              Picks open
            </span>
          )}
          {preSeason && (
                      ...
                      <td className="px-4 py-2.5 tabular-nums">
                        {isTop3 ? (
                          <span
                            className={
                              r.position === 1
                                ? "text-[var(--color-racing-yellow)] font-bold"
                                : r.position === 2
                                  ? "text-[var(--color-garage-metal)] font-bold"
      ...
      {isAdmin && !race.resultsLocked && (
        <section className="garage-card">
          ...
          <p className="text-sm text-[var(--color-oil-stain)]">
            Go to{" "}
            <Link href="/admin" className="text-[var(--color-racing-red)] hover:underline">
              Admin
            </Link>{" "}
